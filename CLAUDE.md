...
- **Preço dos itens:** todo item tem `unitPriceCents` numérico, **padrão 0** (nunca `undefined`/`null`); 0 = badge "Definir preço". O **total do topo é a soma só dos itens marcados "Pego"** (`cartTotalCents`); a meta é comparada com esse total (`estimatedTotalCents` = todos os itens continua exportado). Unidades: `un`, `kg`, `g` e `pct` (Pacote: contagem inteira, preço por pacote, mesma fórmula de `un`; migração v7) — em `kg` e `g` a `quantity` está em **gramas** e o preço é **por kg** (mesma fórmula); `g` só muda a exibição ("500 g"). **Lista inicial:** só é criada quando NÃO existe nenhuma lista (`ensureInitialList`, single-flight, em TODAS as builds): "Lista de compras", 41 itens, preços 0, desmarcados; nunca substitui nem duplica listas existentes; "Limpar todos os dados" apaga as listas e o `reload()` recria a lista inicial (2026-09-19). **Menu inferior fixo:** telas de aba sem folga inferior (`ScreenContainer` `bottomInset` false), barra com altura fixa (`sizes.tabBar` + inset) e sem `removeClippedSubviews`. **Tocar na linha do item abre o modal de preço** (o checkbox só marca) e **Salvar Preço atualiza o total do topo**. **No modal de preço o nome do item é um campo editável (obrigatório, máx. 80) e o tipo de quantidade é um `Dropdown` (Unidade/Quilo/Gramas/Pacote) que converte a quantidade; tudo é gravado numa única chamada `updateItem`** (2026-09-19).
...
- **Somente tema claro:** o app ignora o modo escuro do sistema (`userInterfaceStyle: light` + `expo-system-ui` + `Appearance.setColorScheme('light')` em `app/_layout.tsx`); não use `useColorScheme` nem crie tema escuro por enquanto (2026-09-19).
