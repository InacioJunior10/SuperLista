...
## Tarefas
1. **Rota modal** `/preco/[itemId]` (`presentation: "formSheet"` ou `"modal"`), abre em < 100 ms sem sair do contexto da lista.
...
7. **Toggle** "Marcar como pego no carrinho" (soma imediatamente ao total da lista).
8. **Rodapé**: `Sair` (descarta alterações pendentes, com confirmação se houver) e `Salvar Preço` (grava no SQLite, atualiza o item e recalcula o total; fecha o modal).
