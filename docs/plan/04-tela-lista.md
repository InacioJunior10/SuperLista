...
2. **Card de Orçamento (hero)**: total do topo = **soma só dos itens marcados "Pego"** (`cartTotalCents`; itens com preço 0 somam 0) em R$ com `currencyDisplay`, "5 de 12 pegos" + `ProgressBar`, "Meta: R$ 200,00" com cor pelo `budgetStatus`, botões **Recalcular** (reset de marcações, com confirmação) e **+ Item**.
...
5. **Total em tempo real** (RF-01): o hero reflete a soma dos itens pegos em toda alteração (marcar/desmarcar, preço, quantidade, unidade, adicionar, remover, editar, recalcular, meta). Persistência imediata.
...
- Rolagem fluida com a lista inicial (41 itens) e com 200 itens de teste.
