import { estimatedTotalCents } from "./totals";
import type { ShoppingList } from "../../types/list";

export type BudgetLevel = "none" | "ok" | "warning" | "over";

export type BudgetStatus = {
  /** Percentual (inteiro arredondado) do total estimado sobre a meta; 0 sem meta. */
  percent: number;
  /** Meta menos total estimado (negativo se estourou); 0 sem meta. */
...
export function budgetStatus(list: Pick<ShoppingList, "items" | "budgetCents">): BudgetStatus {
  ...
  const total = estimatedTotalCents(list);
